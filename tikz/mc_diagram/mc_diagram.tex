...
%nodes
\node[solidbox] (mc_generation) {\centering {\textbf MC generation} \\
	\begin{center}
...
\node[solidbox, right=of mc_generation] (detector_simulation) {\centering {\textbf Detector simulation} \\
    \begin{center}
	Smearing to mimic\\ detector response.
	\end{center}
};

\node[solidbox,right=of detector_simulation] (reweighting) {\centering {\textbf Reweighting} \\
...
\node[solidbox,below=of reweighting] (mc_histogram) {\centering {\textbf Histogram MC} \\
	Binned MC expectation.
};

\node[solidbox, below=of mc_generation] (data_generation) {\centering {\textbf Synthetic data generation} \\
...
\node[solidbox, right=of data_generation] (data_detector_simulation) {\centering {\textbf Detector simulation} \\
    \begin{center}
	Smearing to mimic\\ detector response.
	\end{center}
};

\node[solidbox,below=of mc_histogram] (data_histogram) {\centering {\textbf Histogram Data} \\
	Binned synthetic data.
};

\node[circle, minimum size=1cm, color=black, draw=black, very thick, right=of data_histogram] (likelihood) {\centering {\textbf $\mathcal{L}(\vec\theta)$}
};
