...
Similarly to the calculation techniques used for the analysis in this work, the cascade selection analysis computed the effect of accompanying muons with the older passing fraction calculation described in~\cite{Gaisser:2014bja}.
...
While this treatment is sufficient for the current analysis of the cascade sample, more accurate calculations of the atmospheric suppression are desired for a global analysis.
...
An easier first step may be to use the less sensitive medium energy starting event sample, for which more accurate background estimation are more straightforward to obtain.

\subsubsection{Medium-energy starting events}
The medium-energy starting event sample modifies the veto definitions of the HESE selection and extends the selection to lower energies primarily by applying a charge dependent fiducial volume cut.
The size of the fiducial volume is reduced for lower charge events to compensate for the smaller amount of light that lower energy muons produce per unit length.

The medium-energy starting event selection has similar properties to high-energy starting event selection in that only events with a contained interaction vertex are selected.
The sample consists mostly of cascades, and the there is a separation between veto and fiducial volume.
